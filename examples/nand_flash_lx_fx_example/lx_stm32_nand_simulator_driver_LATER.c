...
static NAND_FLASH_BLOCK nand_memory_area[TOTAL_BLOCKS];
static NAND_BLOCK_DIAG nand_block_diag[TOTAL_BLOCKS];

/* Define NAND flash buffer for LevelX.  */
static ULONG nand_flash_simulator_buffer[WORDS_PER_PHYSICAL_PAGE];
static ULONG *nand_flash_memory;

static UINT lx_nand_simulator_read(ULONG block, ULONG page, ULONG *destination, ULONG words);
static UINT lx_nand_simulator_write(ULONG block, ULONG page, ULONG *source, ULONG words);
static UINT lx_nand_simulator_block_erase(ULONG block, ULONG erase_count);
static UINT lx_nand_simulator_block_erased_verify(ULONG block);
static UINT lx_nand_simulator_page_erased_verify(ULONG block, ULONG page);
static UINT lx_nand_simulator_erase_all(VOID);
static UINT lx_nand_simulator_block_status_get(ULONG block, UCHAR *bad_block_byte);
static UINT lx_nand_simulator_block_status_set(ULONG block, UCHAR bad_block_byte);
static UINT lx_nand_simulator_extra_bytes_get(ULONG block, ULONG page, UCHAR *destination, UINT size);
static UINT lx_nand_simulator_extra_bytes_set(ULONG block, ULONG page, UCHAR *source, UINT size);
static UINT lx_nand_simulator_system_error(UINT error_code, ULONG block, ULONG page);
